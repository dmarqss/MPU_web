  ...
  <div class="auth-container">
    <div class="login-card">
        ...
        <div class="mb-3">
            ...
            <input v-model="newPassword" type="password" id="inputPassword5" class="form-control" aria-describedby="passwordHelpBlock">
        </div>
        <div class="mb-3">
            <label for="inputPassword5" class="form-label">Confime a Senha</label>
            <input v-model="newPasswordConfirm" type="password" id="inputPassword5" class="form-control" aria-describedby="passwordHelpBlock">
        </div>
        <div class="d-grid gap-3 d-md-flex justify-content-between">
            <button @click="resetPassword" class="btn btn-primary w-100" type="button">
                Enviar
            </button>
        </div>
    </div>
            ...
            const resetPassword = async () => {
                try{
                    if(newPassword.value === newPasswordConfirm.value){
                        await UserService.resetPassword(route.query.token, newPassword.value)
                        addNotfication('success', 'senha alterada com sucesso')
                        setTimeout( () => {
                            router.push('/login')
                        }, 1000)
                    }else{
                         addNotfication('fail', 'as senhas não são iguais')
                    }
                    

                }catch(error){
                    addNotfication('fail', 'o envio da senha falhou')
